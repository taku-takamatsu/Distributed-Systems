...
func TestBasic(t *testing.T) {
	...
	fmt.Printf("Test: Joined gid=%v servers=%v\n", gids[0], ha[0])
	...
	fmt.Printf("Test: are keys still there after leaves?\n\n")
	// are keys still there after leaves?
	for g := 0; g < len(gids)-1; g++ {
		mck.Leave(gids[g])
		fmt.Printf("Test: Leave gid=%v\n", gids[g])
		time.Sleep(1 * time.Second)
		for i := 0; i < len(keys); i++ {
			v := ck.Get(keys[i])
			if v != vals[i] {
...
func TestLimp(t *testing.T) {
	...
	// are keys still there after joins?
	fmt.Println("are keys still there after joins?")
	for g := 1; g < len(gids); g++ {
		mck.Join(gids[g], ha[g])
		time.Sleep(1 * time.Second)
		for i := 0; i < len(keys); i++ {
	...
	// are keys still there after leaves?
	fmt.Println("are keys still there after leaves?")
	for g := 0; g < len(gids)-1; g++ {
		mck.Leave(gids[g])
		fmt.Printf("Leave gid=%v", gids[g])
...
func doConcurrent(t *testing.T, unreliable bool) {
	...
	for i := 0; i < len(gids); i++ {
		...
		fmt.Printf("Test: Join gid=%v, servers=%v\n", gids[i], ha[i])
	}
